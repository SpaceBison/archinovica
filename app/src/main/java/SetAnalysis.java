...
public class SetAnalysis {
    ...
    public ArrayList<boolean[]> allSets(ArrayList<boolean[]> sets, int noteIndex) {
        ...
        for (boolean[] set : sets) {
            ...
            if (noteIndex >= 0) System.arraycopy(set, 0, copy, 0, noteIndex);
            copy[noteIndex] = false;
            allSets.add(copy);
        }
        for (boolean[] set : sets) {
            boolean[] copy = new boolean[12];
            System.arraycopy(set, 0, copy, 0, noteIndex);
            copy[noteIndex] = true;
            allSets.add(copy);
        }
        if (noteIndex < 11) {
        ...
        for (ArrayList<Basket> sequence : sequences) {
            ...
            for (int i = 0; i < sequence.size(); i++) {
                ...
                if (i == 0) {
                    ...
                    firstPitch[0] /= chord.size();
                    firstPitch[1] /= chord.size();
                }
                if (i == sequence.size() - 1) {
                    ...
                    lastPitch[0] /= chord.size();
                    lastPitch[1] /= chord.size();
                }
            }
